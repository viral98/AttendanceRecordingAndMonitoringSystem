      ...
      <main class="mdl-layout__content mdl-color--grey-100">
        ...
        <?php
          ...
          if (isset($_GET["search"]))
          {
              ...
              {
                $subcode=$_GET["SubCode"];
                $sqlsubcodes="SELECT sub_name,lecture,teacher_id FROM student.subject where sub_code = '$subcode' " ;
                $result = $conn->query($sqlsubcodes);
                $row=$result->fetch_assoc();
                $teachid=$row["teacher_id"];

                $sqlsubcode="SELECT teach_name FROM student.teacher where teach_id = '$teachid' " ;
                $result1 = $conn->query($sqlsubcode);
                $row1=$result1->fetch_assoc();
                echo "Subject Name : ".$row["sub_name"]."<br>";
                echo "Teacher Name: ".$row1["teach_name"]."<br>"."Total Lectures :".$row["lecture"];
              ...
              else if (!empty($subcode) and !empty($rollno) and !empty($datefrom) and !empty($dateto))
              {
                ...
                else if (!empty($subcode) and empty($rollno) and !empty($datefrom) and !empty($dateto))
                {
                    ...
                    $sqlsubcodes="SELECT CAST(datetime1 AS DATE) AS datetime1,COUNT(status) as cnt FROM student.attendance where datetime1 > '$datefrom' and datetime1 < '$dateto' AND attend_sub_code='$subcode' ";
                    $result = $conn->query($sqlsubcodes);
                    $row=$result->fetch_assoc();
                    echo "Total Students Record :".$row["cnt"]."<br>";

                    $sqlsubcodes="SELECT CAST(datetime1 AS DATE) AS datetime1,COUNT(status) as cnt FROM student.attendance where datetime1 > '$datefrom' and datetime1 < '$dateto' AND attend_sub_code='$subcode' and status='1' ";
                    $result = $conn->query($sqlsubcodes);
                    $row=$result->fetch_assoc();
                    echo "Total Student Present :".$row["cnt"]."<br>";

                    $sqlsubcodes="SELECT CAST(datetime1 AS DATE) AS datetime1,COUNT(status) as cnt FROM student.attendance where datetime1 > '$datefrom' and datetime1 < '$dateto' AND attend_sub_code='$subcode' and status='0' ";
                    $result = $conn->query($sqlsubcodes);
                    $row=$result->fetch_assoc();
                    echo "Total Student Absent :".$row["cnt"]."<br>";

    ...
    <div class="demo-layout mdl-layout mdl-js-layout mdl-layout--fixed-drawer mdl-layout--fixed-header">
      <header class="demo-header mdl-layout__header mdl-color--grey-100 mdl-color-text--grey-600">
        <div class="mdl-layout__header-row">
          <span class="mdl-layout-title">Modify Attendance</span>
      ...
      <main class="mdl-layout__content mdl-color--grey-100">
        ...
        <br>
          ...
          <div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label">
            <input class="mdl-textfield__input" type="number" pattern="-?[0-9]*(\.[0-9]+)?" id="status" name="status">
        ...
        <?php
          ...
          if (isset($_GET["search"]))
          {
              ...
              $sqlteach_id="SELECT teach_id FROM student.teacher where teach_name='$user'";
              $result = $conn->query($sqlteach_id);
              $row = $result->fetch_assoc();
              $teachid=$row["teach_id"];

              $sqlsub="SELECT sub_code FROM student.subject where teacher_id='$teachid'";
              $result = $conn->query($sqlsub);
              while($row=$result->fetch_assoc()) 
                  {
                      if($row["sub_code"]==$subcode)
                      {
                          $sqlrollnosubcode="UPDATE student.attendance SET status=$status where attend_stud_id = '$rollno' AND attend_sub_code = '$subcode' AND datetime1 like '{$date}%' ";
                          if($result = $conn->query($sqlrollnosubcode)==TRUE)
                          {
                                  echo "SUCCESSFULL";
                                  $s=1;
                                  break;
                  ...
                  if($s!=1)
				  {
                    echo "Not Successfull or No permission"."<br>";
                    echo "Please Contact the Administator";
				  }

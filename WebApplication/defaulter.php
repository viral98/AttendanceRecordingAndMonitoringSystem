    ...
    <div class="demo-layout mdl-layout mdl-js-layout mdl-layout--fixed-drawer mdl-layout--fixed-header">
      <header class="demo-header mdl-layout__header mdl-color--grey-100 mdl-color-text--grey-600">
        <div class="mdl-layout__header-row">
          <span class="mdl-layout-title">Defaulters</span>
      ...
      <?php
          ...
          if ($conn->connect_error) {
           ...
           if (isset($_GET["search"]))
           {
              ...
              $query="SELECT lecture,sub_name,teacher_id from subject where sub_code= $subcode";
              $totallec = $conn->query($query);
              $total = $totallec -> fetch_assoc();
              $totalfinal = (int)$total["lecture"];
              $subname = $total["sub_name"];
              $teacherid = (int)$total["teacher_id"];
              $queryn="SELECT teach_name from teacher where teach_id=$teacherid";
              $totallecn = $conn->query($queryn);
              $totaln = $totallecn -> fetch_assoc();
              $tname=$totaln["teach_name"];
              echo "Teacher Id : " . $teacherid."&nbsp &nbsp &nbsp &nbsp &nbsp &nbsp";
              echo "Teacher Name : " . $tname."<br>";
              echo "Subject Name : " . $subname."<br>";
              echo "Total Lectures : " . $totalfinal."<br>";
              $totalcount= 0;
                $subcode=$_GET["SubCode"];
                ...
                if($result->num_rows > 0)
                {
                    while ($row=$result->fetch_assoc()) {
                      ...
                      $sqlrollnosubcode="SELECT attend_stud_id,COUNT(*) as NumLec FROM student.attendance where attend_sub_code = '$subcode' and attend_stud_id= '$a' and status = 1";
                            ...
                            if((($row1["NumLec"]/$totalfinal)*100) < 75)      
                            {               
